...
//        renewItems(sliderView);
    ...
    private void loadAnimeData() {
        db.collection("Anime").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                for (QueryDocumentSnapshot document : task.getResult()){
//                    String id = document.getId();
                    String thumb= document.getData().get("Thumb").toString();
...
////                Reference thumb = documentSnapshot.getReference("Thumb");
////                DocumentReference thumb = documentSnapshot.getDocumentReference("Thumb");
//                String thumb = documentSnapshot.getString("Thumb");
//                StorageReference thumbRef = storageRef.child(thumb);
//                thumbRef.getDownloadUrl().addOnSuccessListener(uri -> {
//                    String thumbUrl = uri.toString();
//                    MovieModel movieModel1 = new MovieModel(thumbUrl, title);
...
//        });
    ...
    private void loadCartoonData() {
        db.collection("Cartoon").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                for (QueryDocumentSnapshot document : task.getResult()) {
                    String id = document.getId();
